...
const [file, setFile] = useState("");
  ...
  const getDataEvents = async (idd) => {
    try {
      const docRef = doc(db, "produk_labtool", idd);
...
const addData = async (e) => {
    e.preventDefault();
    const todoRef = doc(db, "produk_labtool", id);
...
<div></div>
           ...
           <div className="flex flex-col gap-3">
            ...
            {loading? <></>:<>
            <button className="p-3 text-white rounded-md bg-blue-500" onClick={addData}>EDIT</button>
            </>}
           </div>
        </div>
      </div>
