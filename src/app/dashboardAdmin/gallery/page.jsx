...
function GalleryAdmin() {
  ...
  return (
    <>
      <div className="flex">
        <Navigation clients="ps-3 text-[#184737] bg-white rounded-sm" />
          ...
          <div className="w-full bg-[#184737] ">
            ...
            <div className="p-5 w-full">
              <div className="grid md:grid-cols-5 sm:grid-cols-3 grid-cols-1 gap-7">
                 <a
                  href="/dashboardAdmin/gallery/create/"
                ...
                {dataClient.map((data, i) => {
                  return (
                    <>
                      <div key={i} className="border-2 flex flex-col p-2 rounded-md">
                        <div
                          className="h-32 bg-white rounded-md p-2 bg-contain bg-center bg-no-repeat"
                          style={{ backgroundImage: `url(${data.img})` }}
                        ></div>
                        <div className="text-white line-clamp-2"><p>{data.title}</p></div>
                        <div className=" flex  pt-2">
                          {/* <a
                            href={`/dashboardAdmin/clients/edit?id=${data.id}`}
                            className="bg-yellow-400 h-10 rounded-md w-full p-3 flex items-center justify-center"
                          >
                            <Image
                            ...
                            onClick={async (e) => {
                              ...
                              if (confirmed) {
                                try {
                                  ...
                                  await deleteDoc(doc(db, "galleries", data.id));
